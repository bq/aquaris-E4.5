...
nlmsg_failure:
	kfree_skb(skb);
	*errp = -EINVAL;
	printk(KERN_ERR "[mtk_net][ip_queue]: error creating packet message\n");
	return NULL;
}
...
ipq_enqueue_packet(struct nf_queue_entry *entry, unsigned int queuenum)
{
	...
	if (queue_total >= queue_maxlen) {
		...
		{	
			...
			printk (KERN_WARNING "[mtk_net][ip_queue]: full at %d entries, "
...
ipq_mangle_ipv4(ipq_verdict_msg_t *v, struct nf_queue_entry *e)
{
	...
	if (diff < 0) {
		...
		if (diff > skb_tailroom(e->skb)) {
			...
			if (!nskb) {
				#ifdef CONFIG_MTK_NET_LOGGING  
				printk(KERN_WARNING "[mtk_net][ip_queue]: error "
...
static int __init ip_queue_init(void)
{
	...
	if (ipqnl == NULL) {
		printk(KERN_ERR "[mtk_net][ip_queue]: failed to create netlink socket\n");
	...
	proc = proc_create(IPQ_PROC_FS_NAME, 0, init_net.proc_net,
			   &ip_queue_proc_fops);
	if (!proc) {
		printk(KERN_ERR "[mtk_net][ip_queue]: failed to create proc entry\n");
		goto cleanup_ipqnl;
	}
#endif
	...
	if (status < 0) {
		printk(KERN_ERR "[mtk_net][ip_queue]: failed to register queue handler\n");
		goto cleanup_sysctl;
	}
	return status;
