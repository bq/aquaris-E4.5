	...
	if (0 == copy_from_user(local, buf, (count > sizeof(local)) ? sizeof(local) : count)) {
	    ...
	    if (local[0] == '0') {
			...
			while (netdev == NULL && wait_cnt < 10) {
				WIFI_WARN_FUNC("Fail to get wlan0 net device, sleep 300ms\n");
				msleep(300);
				wait_cnt ++;
				netdev = dev_get_by_name(&init_net, WLAN_IFACE_NAME);
			}
			if (wait_cnt >= 10) {
				WIFI_WARN_FUNC("Get wlan0 net device time out\n");
				goto done;
			}
			...
			if (pf_set_p2p_mode) {
				if (pf_set_p2p_mode(netdev, p2pmode) != 0){
					WIFI_WARN_FUNC("Turn off p2p/ap mode fail");
					goto done;
				}
			}
			dev_put(netdev);
			...
			int k = 0;
			/*0: no ddebug, 1: common debug output, 2: more detials  3: verbose*/
			switch (local[1]) {
			case '0':
			...
			case '1':
				...
				wlan_dbg_level[DBG_TX_IDX] &= ~(DBG_CLASS_TRACE | \
												DBG_CLASS_INFO | \
												DBG_CLASS_EVENT);
				wlan_dbg_level[DBG_RX_IDX] &= ~(DBG_CLASS_TRACE | \
												DBG_CLASS_INFO | \
												DBG_CLASS_EVENT);
				wlan_dbg_level[DBG_REQ_IDX] &= ~(DBG_CLASS_TRACE | \
												DBG_CLASS_INFO | \
												DBG_CLASS_EVENT);
			...
			if (pf_set_p2p_mode == NULL) {
				WIFI_INFO_FUNC("Set p2p mode handler is NULL\n");
				goto done;
			}
			...
			if (local[0] == 'S' || local[0] == 'P') {
				...
				p2pmode.u4Mode = 0;
				if (pf_set_p2p_mode(netdev, p2pmode) != 0){
					WIFI_WARN_FUNC("Set wlan mode fail\n");
				}
				else{
