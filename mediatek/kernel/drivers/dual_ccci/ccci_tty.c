...
static ssize_t ccci_tty_read(struct file *file, char *buf, size_t count, loff_t *ppos)
{
	...
	if( (read + data_be_read) >= length ) {
		...
		// Copy first part
		part = length - read;
		ret = copy_to_user(u_buf,&rx_buffer[read],part);
		if (ret) {
  		CCCI_MSG_INF(md_id, "tty", "read: copy_to_user fail:u_buf=0x%08x,rx_buffer=0x%08x,read=%d,size=%d ret=%d line=%d\n",(unsigned int)u_buf,(unsigned int)rx_buffer,read,part,ret,__LINE__);

			ret= -EFAULT;
			goto out;
		}
		// Copy second part
		ret = copy_to_user(&u_buf[part],rx_buffer,data_be_read - part);
		if (ret) {
		...
		// Just need read once
		ret =copy_to_user(u_buf,&rx_buffer[read],data_be_read);
...
static ssize_t ccci_tty_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if( (write+data_be_write) >= length) {
		...
		if (ret)
		{
			CCCI_MSG_INF(md_id, "tty", "write: copy from user fail:buf=0x%08x,tx_buffer=0x%08x,write=%d,size=%d ret=%d line=%d\n",\
							(unsigned int)buf,(unsigned int)tx_buffer,write,part,ret,__LINE__);
			ret=-EFAULT;
			goto out;
		}
		// 2nd write
		...
		{
			CCCI_MSG_INF(md_id, "tty", "write: copy from user fail:buf=0x%08x,tx_buffer=0x%08x,write=%d,size=%d ret=%d line=%d\n",\
							(unsigned int)buf,(unsigned int)tx_buffer,data_be_write,part,ret,__LINE__);
			ret=-EFAULT;
			goto out;
		}
	}
