...
static EEMCS_EXCEPTION_STATE eemcs_exception_state = EEMCS_EX_INVALID;
...
kal_bool set_exception_mode(EEMCS_EXCEPTION_STATE new_mode)
{
    ...
    KAL_ASSERT((new_mode >= EEMCS_EX_NONE) && (new_mode <= EEMCS_EX_INIT_DONE));
    ...
    switch(msg->id){
       case MD_EX :
            if(msg->reserved != MD_EX_CHK_ID){
                DBGLOG(EXPT, ERR, "[EXPT] Invalid MD_EX. reserved expect(%#X) CCCI_H(%#X)(%#X)(%#X)(%#X)",
                MD_EX_CHK_ID, msg->magic, msg->id, msg->channel, msg->reserved);
                return KAL_FAIL;
            }
        ...
        case MD_EX_REC_OK :
            if(msg->reserved != MD_EX_REC_OK_CHK_ID){
                DBGLOG(EXPT, ERR, "[EXPT] Invalid MD_EX_REC_OK. reserved expect(%#X) CCCI_H(%#X)(%#X)(%#X)(%#X)",
                MD_EX_REC_OK_CHK_ID, msg->magic, msg->id, msg->channel, msg->reserved);
                return KAL_FAIL;
            }
            ...
            DBGLOG(EXPT, DBG, "eemcs_ee_info_dump");
        ...
        case CCCI_DRV_VER_ERROR :
            DBGLOG(EXPT, DBG, "AP EEMCS driver version mis-match to MD!!");
            eemcs_aed(0, 0, "AP/MD driver version mis-match\n");
            // TODO:  stop retry boot md
            break;
            
        default:
            DBGLOG(EXPT, ERR, "[EXPT] Invalid MD_EX_MAGIC. CCCI_H(%#X)(%#X)(%#X)(%#X)",
