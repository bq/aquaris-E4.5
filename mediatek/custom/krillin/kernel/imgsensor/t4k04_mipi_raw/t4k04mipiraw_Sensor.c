...
#ifdef T4K04_OTP_FUNC_SUPPORT
#define T4K04_OTP_PSEL				0X3102
#define T4K04_OTP_ENABLE			0X3100
#define T4K04_OTP_DATA_BEGIN_ADDR	0X3104
#define T4K04_OTP_DATA_END_ADDR		0X3143
#define T4K04_OTP_PCLK_SETTING		0X3145

kal_uint16	r_otp_gold, g_otp_gold, b_otp_gold, r_otp, g_otp, b_otp, rg, bg;

void T4K04_otp_read(unsigned short page, unsigned char *otp_data);
bool T4K04_update_otp_wb(void);
bool T4K04_update_otp_lenc(void);
bool T4K04_otp_check_page(unsigned char *otp_data);
bool T4K04_otp_checksum(unsigned char *otp_data, int size);
void T4K04_otp_apply_lenc(unsigned char *otp_data);
#endif
...
void T4K04_otp_read(unsigned short page, unsigned char *otp_data)
{
	...
	//enable controls access to OTP
	T4K04_write_cmos_sensor(T4K04_OTP_ENABLE, 0x01);
	
	//set OTP clk  (mclk = 24M)
	T4K04_write_cmos_sensor(T4K04_OTP_PCLK_SETTING, 0x06);
	
	//set page to read
	T4K04_write_cmos_sensor(T4K04_OTP_PSEL, page);	//page 2
	
	//page to buffer 
	T4K04_write_cmos_sensor(T4K04_OTP_ENABLE, 0x81);	//bit 7 -> 1

	//wait for 30us or more
	mDELAY(50);
	
	//read otp data from buffer
	otp_addr = T4K04_OTP_DATA_BEGIN_ADDR;		//buffer addr
	for (i=0; i<64; i++) 
	{
		otp_data[i] = T4K04_read_cmos_sensor(otp_addr);
		otp_addr++;
	}

	//set backup page to read
	T4K04_write_cmos_sensor(T4K04_OTP_PSEL, page+6);
	...
	for (i=0; i<64; i++) 
	{
		otp_data[i] |= T4K04_read_cmos_sensor(otp_addr+i);
	}

	//disable OTP
	T4K04_write_cmos_sensor(T4K04_OTP_ENABLE, 0x00);	

}

bool T4K04_update_otp_wb(void) 
{
	unsigned char otp_data[64];
	unsigned char *otp_data_awb;

	T4K04DB("start wb update\n");

	//golden module AWB para area
	T4K04_otp_read(0x04, otp_data);

	if (otp_data[32] == 0)		//this page has no OTP data 
	{
		if (otp_data[0] == 0)
		{
			T4K04DB(" error : no OTP data !\n ");
			return KAL_FALSE;
		}
		else
		{
			otp_data_awb = &otp_data[0];				
		}
	}
	else
	{	
		otp_data_awb = &otp_data[32];
	}
	
	if (!T4K04_otp_checksum(otp_data_awb, 32))
	{
		T4K04DB(" error : wb checksum wrong!\n ");
		return KAL_FALSE;
	}
	#if 0 //zh modify to fixed golden sample awb value20130327
	r_otp_gold = ((((kal_uint16)otp_data_awb[15]<<8)&0xFF00)|(((kal_uint16)otp_data_awb[16])&0x00FF));
	g_otp_gold = (((((kal_uint16)otp_data_awb[17]<<8)&0xFF00)|(((kal_uint16)otp_data_awb[18])&0x00FF))+((((kal_uint16)otp_data_awb[19]<<8)&0xFF00)|(((kal_uint16)otp_data_awb[20])&0x00FF)))/2;
	b_otp_gold = ((((kal_uint16)otp_data_awb[21]<<8)&0xFF00)|(((kal_uint16)otp_data_awb[22])&0x00FF));
	#else
	r_otp_gold=0x67;
	g_otp_gold=0x93;
	b_otp_gold=0x71;
	#endif
	rg = 256 * (r_otp_gold * g_otp)/(r_otp * g_otp_gold);
	bg = 256 * (b_otp_gold * g_otp)/(b_otp * g_otp_gold);

	T4K04DB(" --------tl--------rg = 0x%x, bg =0x%x\n ",rg,bg);
	T4K04DB("Year:%x,Month:%x,Date:%x\n", otp_data_awb[3],otp_data_awb[4],otp_data_awb[5]);
	T4K04DB("Module Vendor:%x\n", otp_data_awb[6]);
	T4K04DB("Version:%x\n", otp_data_awb[7]);
	T4K04DB("VCM:%x\n", otp_data_awb[8]);
	T4K04DB("VCM Driver:%x\n", otp_data_awb[9]);
	T4K04DB("Lens:%x\n", otp_data_awb[10]);
	T4K04DB("OTP Page Mapping:%x\n", otp_data_awb[11]);
		
	T4K04_write_cmos_sensor(0X0210, (rg>>8)&0x03);
	T4K04_write_cmos_sensor(0X0211, rg&0xff);
	T4K04_write_cmos_sensor(0X0212, (bg>>8)&0x03);
	T4K04_write_cmos_sensor(0X0213, bg&0xff);
	
	T4K04DB(" --------tl--------0x0210 = 0x%x\n ",T4K04_read_cmos_sensor(0x0210));
	T4K04DB(" --------tl--------0x0211 = 0x%x\n ",T4K04_read_cmos_sensor(0x0211));
	T4K04DB(" --------tl--------0x0212 = 0x%x\n ",T4K04_read_cmos_sensor(0x0212));
	T4K04DB(" --------tl--------0x0213 = 0x%x\n ",T4K04_read_cmos_sensor(0x0213));

	T4K04DB(" wb update finished\n");
	return KAL_TRUE;
...
bool T4K04_update_otp_lenc(void) 
{
	...
	T4K04DB("start lenc update\n");

	if (T4K04_otp_check_page(otp_data))
	{
		if (T4K04_otp_checksum(otp_data, 64))
		{
			r_otp = ((((kal_uint16)otp_data[56]<<8)&0xFF00)|(((kal_uint16)otp_data[57])&0x00FF));
			g_otp = (((((kal_uint16)otp_data[58]<<8)&0xFF00)|(((kal_uint16)otp_data[59])&0x00FF))+((((kal_uint16)otp_data[60]<<8)&0xFF00)|(((kal_uint16)otp_data[61])&0x00FF)))/2;
			b_otp = ((((kal_uint16)otp_data[62]<<8)&0xFF00)|(((kal_uint16)otp_data[63])&0x00FF));

			T4K04_otp_apply_lenc(otp_data);
			T4K04DB("lenc update finished\n");
			return KAL_TRUE;
		}		
		T4K04DB("error : lenc checksum wrong\n");
		return KAL_FALSE;
	}
...
bool T4K04_otp_check_page(unsigned char *otp_data)
{
	T4K04_otp_read(0x03, otp_data);

	//lens shading
	if (otp_data[0] == 0)		//this page has no OTP data 
	{
		T4K04_otp_read(0x02, otp_data);		//try the other page
		if (otp_data[0] == 0)
		{
			//zh add for OTP modify0131
			T4K04_otp_read(0x01, otp_data);		//try the other page
			if (otp_data[0] == 0)
			{
				T4K04_otp_read(0x00, otp_data);		//try the other page
				if (otp_data[0] == 0)
				{
					T4K04DB(" error : no OTP data !\n ");
					return KAL_FALSE;
...
bool T4K04_otp_checksum(unsigned char *otp_data,  int size)
{
	...
	if(otp_data[1] != checksum)
	{
		T4K04DB(" error : OTP checksum error !\n ");
		return KAL_FALSE;
	}

	T4K04DB(" OTP checksum OK !\n ");
	return KAL_TRUE;
...
void T4K04_otp_apply_lenc(unsigned char *otp_data)
{
	...
	//write data to regs
	T4K04_write_cmos_sensor(0x3283, otp_data[3]);	
	for (i=4; i<=55; i++)
	{
		T4K04_write_cmos_sensor((0x3283+i), otp_data[i]);	
	}
}
#endif /* T4K04_OTP_FUNC_SUPPORT */
...
UINT32 T4K04Open(void)
{
	...
	sensor_id = ((T4K04_read_cmos_sensor(0x0000))<<8)|(T4K04_read_cmos_sensor(0x0001));
	...
	#ifdef T4K04_OTP_FUNC_SUPPORT
	//t4k04 OTP
	if(T4K04_update_otp_lenc())
	{		
		T4K04DB(" OTP lenc update success !\n ");
	}
	
	if(T4K04_update_otp_wb())
	{		
		T4K04DB(" OTP wb update success !\n ");
	}
	#endif
...
UINT32 T4K04GetSensorID(UINT32 *sensorID)
{
    ...
    do {
        *sensorID = ((T4K04_read_cmos_sensor(0x0000))<<8)|(T4K04_read_cmos_sensor(0x0001));
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_lock(&t4k04mipiraw_drv_lock);
	sensor_config_data->SensorImageMirror = IMAGE_NORMAL;//by layout
...
{	
	pSensorConfigData->SensorImageMirror = IMAGE_NORMAL;//by layout
