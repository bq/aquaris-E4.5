...
static struct i2c_board_info __initdata i2c_LIS3DH={ I2C_BOARD_INFO("LIS3DH", (LIS3DH_I2C_SLAVE_ADDR))};
...
static bool gsensor_ctl1_status = false;
//added by ST WU Yi 2013.12.10.
char acc_data_buff[LIS3DH_BUFSIZE] = {0};

static u8 bw = 0x00;
static u8 bw_ctl_reg1_value = LIS3DH_BW_100HZ;
/*----------------------------------------------------------------------------*/
#define GSE_TAG                  "[Gsensor] "
#define GSE_FUN(f)               printk(KERN_INFO GSE_TAG"ACC_DEBUG %s\n", __FUNCTION__)
#define GSE_ERR(fmt, args...)    printk(KERN_ERR GSE_TAG"%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define GSE_LOG(fmt, args...)    printk(KERN_ERR GSE_TAG fmt, ##args)
/*----------------------------------------------------------------------------*/
void getTimestamp(long long *sec,long long *usec) {
    struct timeval tv;
...
static int LIS3DH_ReadData(struct i2c_client *client, s16 data[LIS3DH_AXES_NUM])
{
	...
	{
            if(hwmsen_read_block(client, AUTO_INCREMENT|LIS3DH_REG_OUT_X, buf, 6))
	    {
...
static int LIS3DH_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(enable == TRUE)
	{
		databuf[0]  =  bw_ctl_reg1_value;
		databuf[0] |=  LIS3DH_MEASURE_MODE;
	  GSE_ERR("--ACC --inpower enable bw_ctl_reg1_value = %x databuf[0] is %x\n",bw_ctl_reg1_value,databuf[0]);
	}
	else
	{
		databuf[0] &= LIS3DH_MEASURE_MODE;
		 GSE_ERR("--ACC --inpower diable bw_ctl_reg1_value = %x databuf[0] is %x\n",bw_ctl_reg1_value,databuf[0]);
	}
	databuf[1] = databuf[0];
...
static int LIS3DH_SetBWRate(struct i2c_client *client, u8 bwrate)
{
    ...
    bw_ctl_reg1_value = (bwrate | 0x07) & 0xF7;

	databuf[1] = bw_ctl_reg1_value;
	databuf[0] = LIS3DH_REG_CTL_REG1;

    GSE_ERR("--ACC -- bw_ctl_reg1_value = %x bwrate is %x\n",bw_ctl_reg1_value,bwrate);
	res = i2c_master_send(client, databuf, 0x2);
...
*/
    // first clear reg1
    res = hwmsen_write_byte(client,LIS3DH_REG_CTL_REG1,0x07);	//LIS3DH_REG_CTL_REG1		0x20
	if(res != LIS3DH_SUCCESS)
	{
		return res;	//LIS3DH_SUCCESS = 0
	}
...
static int LIS3DH_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if(sensor_suspend == 1)
	{
		GSE_LOG("--ACC--sensor in suspend read not data!\n");
		return 0;
	}
#if 0
...
#endif
      //GSE_ERR("--ACC--sensor in read sensor data!\n");
	if((res = LIS3DH_ReadData(client, obj->data)))
	{
		GSE_ERR("--ACC--I2C error: ret value=%d", res);
		return -3;
	}
	else
	{
...
static int LIS3DH_ReadSensorData1(struct i2c_client *client, char *buf)  //, char *buf, int bufsize)
{
	struct lis3dh_i2c_data *obj = (struct lis3dh_i2c_data*)i2c_get_clientdata(client);
//	u8 databuf[20];
	int acc[LIS3DH_AXES_NUM];
	int res = 0;
        ...
        long long readtime,endtime,difftime;
//	memset(databuf, 0, sizeof(u8)*10);
	...
	if(sensor_suspend == 1)
	{
        ...
        //getTimestamp(&cur_t1,&cur_t2);
        //GSE_ERR("---- LIS3DH DRIVER ST current ready read data reg cur_time %lld %lld\n", cur_t1,cur_t2);
        //readtime =  cur_t2;
		...
		//GSE_LOG("Mapped gsensor data: %d, %d, %d!\n", acc[LIS3DH_AXIS_X], acc[LIS3DH_AXIS_Y], acc[LIS3DH_AXIS_Z]);
          ...
          //    getTimestamp(&cur_t1,&cur_t2);
            // endtime = cur_t2;
             ...
             //GSE_ERR("---- LIS3DH DRIVER ST end read data reg ccur_time %lld %lld\n", cur_t1,cur_t2);
              //difftime =(endtime-readtime);
              //if(difftime > 10000)
              //GSE_ERR("---- LIS3DH DRIVER ST end read delay too long difftime =%lld \n",difftime);
#if DEBUG
		if(atomic_read(&obj->trace) & ADX_TRC_IOCTL)//atomic_read(&obj->trace) & ADX_TRC_IOCTL
		{
			GSE_LOG("gsensor data: err\n" );
			dumpReg(client);
		}
#endif
...
{
       //GSE_ERR("show_acc_data into LIS3DH_ReadSensorData1!!\n");
       return LIS3DH_ReadSensorData1(lis3dh_i2c_client, buf);
}
/*----------------------------------------------------------------------------*/
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			{
				GSE_ERR("--ACC--Set delay parameter error!\n");
				err = -EINVAL;
			}
			else
			{
                                #if ACC_DEBUG
                                GSE_ERR("--ACC--DEBUG Set delay \n");
                                #endif
				value = *(int *)buff_in;
		...
		case SENSOR_ENABLE:
			if((buff_in == NULL) || (size_in < sizeof(int)))
			{
				GSE_ERR("--ACC--Enable sensor parameter error!\n");
				err = -EINVAL;
			}
			else
			{
				...
				value = *(int *)buff_in;
				mutex_lock(&lis3dh_mutex);
				GSE_LOG("--ACC--Gsensor device enable function enable = %d, sensor_power = %d!\n",value,sensor_power);
				if(((value == 0) && (sensor_power == false)) ||((value == 1) && (sensor_power == true)))
				{
					enable_status = sensor_power;
					GSE_LOG("--ACC--Gsensor device have updated!\n");
				}
				else
				{	if(sensor_suspend == 0){
					enable_status = !sensor_power;
					err = LIS3DH_SetPowerMode( priv->client, !sensor_power);
					GSE_LOG("--ACC--Gsensor not in suspend lis3dh_SetPowerMode!, enable_status = %d\n",enable_status);
					}else{
					enable_status = !sensor_power;
					GSE_LOG("--ACC--Gsensor in suspend and can not enable or disable!enable_status = %d\n",enable_status);
					}
				}
				mutex_unlock(&lis3dh_mutex);
			}
		...
		case SENSOR_GET_DATA:
			...
			{
				GSE_ERR("--ACC--get sensor data parameter error!\n");
				err = -EINVAL;
			}
			else
			{
			       //GSE_ERR("--ACC-- SENSOR_GET_DATA in operate!\n");
			...
			break;
		default:
			GSE_ERR("--ACC--gsensor operate function no this parameter %d!\n", command);
			err = -1;
			break;
	}
		...
		case GSENSOR_IOCTL_READ_SENSORDATA:
			...
			if(data == NULL)
			{
				...
				break;
                  ...
                  //GSE_ERR("--Acc -- GSENSOR_IOCTL_READ_SENSORDATA \n");
			if((sensor_power==false)&&(sensor_suspend ==0)){
...
static int lis3dh_suspend(struct i2c_client *client, pm_message_t msg)
{
	...
	if(msg.event == PM_EVENT_SUSPEND)
	{
		...
		dat = dat&0b00000111;   // clear high 5 bit for into powerdownmode
...
static void lis3dh_early_suspend(struct early_suspend *h)
{
	...
	lis_i2c_read_block(obj->client,LIS3DH_REG_CTL_REG1,&data,0x01);
       if((data |0x0f)  == 0x0f)
	   	 gsensor_ctl1_status =false;
	 else
	 	gsensor_ctl1_status  = true;
	 GSE_ERR("lis3dh_early_suspend! gsensor_ctl1_status=%d,data=%x\n",gsensor_ctl1_status,data);

      GSE_ERR("lis3dh_early_suspend!\n");
  	atomic_set(&obj->suspend, 1);
	mutex_lock(&lis3dh_mutex);
	GSE_FUN();
	...
	sensor_suspend = 1;
     GSE_ERR("lis3dh_early_suspend! sensor_suspend=%d\n",sensor_suspend);
	mutex_unlock(&lis3dh_mutex);
	LIS3DH_power(obj->hw, 0);
...
static void lis3dh_late_resume(struct early_suspend *h)
{
	...
	int err;
	   ...
	   GSE_ERR("%s: into lis3dh_late_resume\n", __func__);
      GSE_ERR("%s: into lis3dh_late_resume\n", __func__);
	if(obj == NULL)
	{
		GSE_ERR("null pointer!!\n");
	...
	sensor_suspend = 0;
       LIS3DH_SetPowerMode(obj->client, gsensor_ctl1_status);
	GSE_ERR("%s: into lis3dh_late_resume gsensor_ctl1_status= %d\n", __func__,gsensor_ctl1_status);

	GSE_ERR("set sensor_suspend = 0\n");
...
#ifdef MTK_AUTO_DETECT_ACCELEROMETER
static int lis3dh_probe_comp(void)
{
	...
	if(i2c_probe_ok == -1 ){
		...
		printk("delete lis3dh_i2c_driver \n");
		return -1;
	}
	return 0;
}
/*----------------------------------------------------------------------------*/
static int lis3dh_remove_comp(void)
{
    struct acc_hw *hw = lsm330_get_cust_acc_hw();
...
static struct sensor_init_info mtk_sensor_driver = {
	.name      = "gsensor",
	.init     =   lis3dh_probe_comp,
	.uninit =   lis3dh_remove_comp,
	.create_sys_file = lis3dh_create_attr,
};
...
#ifdef MTK_AUTO_DETECT_ACCELEROMETER
	hwmsen_gsensor_add(&mtk_sensor_driver);
#else
