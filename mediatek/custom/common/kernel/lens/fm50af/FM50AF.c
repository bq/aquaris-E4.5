...
static int s4FM50AF_WriteReg_Common(u16 a_u2Data)
{
	int  i4RetValue = 0;

	char puSendCmd[2] = {(char)(a_u2Data >> 8), (char)(a_u2Data & 0xFF)};// MSB first, then LSB

    FM50AFDB("[FM50AF] g_sr %x, write common %x \n", g_sr, a_u2Data);
    i4RetValue = i2c_master_send(g_pstFM50AF_I2Cclient, puSendCmd, 2);

    if (i4RetValue < 0)
    {
        FM50AFDB("[FM50AF] I2C common send failed!! \n");
        return -1;
    }
...
static int FM50AF_Open(struct inode * a_pstInode, struct file * a_pstFile)
{
	...
	s4FM50AF_WriteReg_Common(0xECA3);//  Ringing ON
	s4FM50AF_WriteReg_Common(0xA10D);//  DLC[3]=b?,  MCLK[1:0]=b?1
	s4FM50AF_WriteReg_Common(0xF200);//  T_SRC[4:0]=b?0100, Tvib/2=5.8ms 
	s4FM50AF_WriteReg_Common(0xDC51);//  Ringing Setting OFF
