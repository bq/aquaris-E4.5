...
void cust_init_key (unsigned char *rsa_n, unsigned int n_len, 
    unsigned char *rsa_d, unsigned int d_len, 
    unsigned char *rsa_e, unsigned int e_len)
{
    return;
}
...
int cust_sign (unsigned char* d_buf,  unsigned int d_len, unsigned char* s_buf, unsigned int s_len)
{
    //RSA1024
	if(s_len == RSA1024_KEY_LENGTH)
	{
