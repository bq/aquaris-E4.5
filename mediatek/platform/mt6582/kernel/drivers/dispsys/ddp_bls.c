...
// 延时中10000次在6572上通过示波器测试是135us左右,则1us就是74次 苏 勇 2013年04月23日 18:30:54
// 在82中,使用74延时约为0.78us,因此使用105 苏 勇 2013年07月24日 11:32:27
#if defined (MT6572)  // 苏 勇 2013年07月24日 11:42:24
#define COUNT_FOR_DELAY (74)
#elif defined(MT6582)
#define COUNT_FOR_DELAY (105)   
#else
#error "老兄,最好去测试下延时次数"   
#endif /* MT6572 */

#define DELAY_BLS_US(time) do \
{\
	volatile int i =0;\
	int temp=(time)*COUNT_FOR_DELAY;\
	for(i=0;i<temp;i++)\
	{\
	};\
}while (0)
...
int disp_bls_set_backlight(unsigned int level)
{
	...
	printk("[LED_luosen]GPIO_num=%d:level=%d\n", gpio_num, level);
	mutex_lock(&backlight_mutex);
	...
	else
	{
...
#endif
		level = 31 - level;
              mdelay(10);//add by luosen 加延时避免LCD刷新过慢造成闪屏
		printk("[LED_luosen]level001=%d,pre_state=%d,pre_level=%d\n", level, pre_state, pre_level);
		if(pre_state == 0)
		{
			if(level < 32 && level >= 0)
			{
		...
		else
		{
			pre_state = 1;
			cur_level = level;
			printk("[luosen]backlight on >>> pre_level: %d >>> cur_level: %d\n", pre_level, cur_level);
 ...
 cur_hall_light_level = cur_level;
 #endif
	return 0;
}
