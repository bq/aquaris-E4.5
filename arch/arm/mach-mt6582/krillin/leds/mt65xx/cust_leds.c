...
int leds_set_button_backlight(unsigned int level)
{
	if(level)
	{	
		#ifdef GPIO_KEY_BACKLIGHT_CTRL_PIN
		printk("[lhj button-backlight level:on %d\n]",level);
		mt_set_gpio_mode(GPIO_KEY_BACKLIGHT_CTRL_PIN,GPIO_MODE_GPIO);
		mt_set_gpio_dir(GPIO_KEY_BACKLIGHT_CTRL_PIN,GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_KEY_BACKLIGHT_CTRL_PIN, GPIO_OUT_ONE);
		#endif
	}
	else
	{
		#ifdef GPIO_KEY_BACKLIGHT_CTRL_PIN
		printk("[lhj button-backlight level:off\n]");
		mt_set_gpio_mode(GPIO_KEY_BACKLIGHT_CTRL_PIN,GPIO_MODE_GPIO);
		mt_set_gpio_dir(GPIO_KEY_BACKLIGHT_CTRL_PIN,GPIO_DIR_OUT);
		mt_set_gpio_out(GPIO_KEY_BACKLIGHT_CTRL_PIN, GPIO_OUT_ZERO);
		#endif
	}
	return 0;
}
