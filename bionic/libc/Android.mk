...
# =========================================================
  ifeq ($(TARGET_BUILD_VARIANT),eng)
    ifeq ($(filter banyan_addon banyan_addon_x86,$(TARGET_PRODUCT)),)
      libc_common_src_files += \
        bionic/fdleak_debug_func.c \
		bionic/munmap.c \
        arch-arm/syscalls/__close.S \
        arch-arm/syscalls/__dup.S \
        arch-arm/syscalls/__pipe.S \
        arch-arm/syscalls/__pipe2.S \
        arch-arm/syscalls/__dup2.S \
        arch-arm/syscalls/__socket.S \
        arch-arm/syscalls/__socketpair.S \
        arch-arm/syscalls/__eventfd.S \
        arch-arm/syscalls/__accept.S \
        arch-arm/syscalls/__munmap.S
      #remove exist file
      libc_common_src_files := $(filter-out arch-arm/syscalls/close.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/dup.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/pipe.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/pipe2.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/dup2.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/socket.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/socketpair.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/eventfd.S, $(libc_common_src_files))
      libc_common_src_files := $(filter-out arch-arm/syscalls/accept.S, $(libc_common_src_files))
	  libc_common_src_files := $(filter-out arch-arm/syscalls/munmap.S, $(libc_common_src_files))
    endif
  endif
...
# Memset Optimization  define compile option by makefile variable
# =========================================================
ifeq ($(MTK_MEMSET0_OPTIMIZE),yes)
  libc_common_cflags += -DHAVE_MADV_ZEROPAGE
endif
...
ifeq ($(TARGET_BUILD_VARIANT),eng)
  ...
  endif # ifeq ($(MTK_USE_RESERVED_EXT_MEM),yes)
...
ifeq ($(TARGET_ARCH),arm)
	...
	LOCAL_SRC_FILES := \
		...
		corkscrew/libc_map_info.c \
		corkscrew/libc_corkscrew_arm.c \
